...
if (!calendarId || !start || !end || !title) return res.status(400).json({ error: 'Missing fields' })
...
return res.json({ ok: true, eventId: event.data.id, htmlLink: event.data.htmlLink })
} catch (err: any) {
console.error(err)
